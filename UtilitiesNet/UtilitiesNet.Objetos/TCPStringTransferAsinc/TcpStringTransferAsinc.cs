...
namespace UtilitiesNet.Objetos.TCPStringTransferAsinc
{
    /// <summary> Conector TCP para el envío de datos, y recepción asincrona </summary>
    public class TcpStringTransferAsinc_OLD : IDisposable
    {
        ...
        public TcpStringTransferAsinc_OLD(string UId, string EndAddress, int EndPort, string ListenerAddress, int ListenerPort)
        {
            uid = UId;
            endPort = EndPort;
    ...
    public class TcpStringTransferAsinc : IDisposable
    {
        ...
        /// <summary>Últimos bytes recibidos</summary>
        public byte[] RawRecivedBytes { get { return bytesClient; } }
        byte[] rawRecivedBytes;
        ...
        public void SendDataToLastServerClient(byte[] message)
        {
            if (LastServerClient != null)
            {
                if (LastServerClient.Connected)
        ...
        private void DataReceived(IAsyncResult ar)
        {
            ...
            bytesClient = bytesClient.Append(byteData.Take(dataRead).ToArray());
            rawRecivedBytes = byteData;
