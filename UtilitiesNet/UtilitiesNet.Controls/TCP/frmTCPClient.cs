...
using UtilitiesNet.Objetos.TCPStringTransferAsinc;
...
namespace UtilitiesNet.Controls.TCP
{
    public partial class frmTCPClient : Form
    {
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                transfer = new TcpStringTransferAsinc(string.Empty, TcpStringTransferAsinc.TipoTrabajo.Client, txtIp.Text, int.Parse(txtPort.Text));
                // transfer.ClientChange += transfer_CambioCliente;
                transfer.DataReaded += transfer_DataReaded;
                transfer.ResponseClient = true;
        ...
        private void btnSendToCli_Click(object sender, EventArgs e)
        {
            if(!string.IsNullOrWhiteSpace(txtEnvio.Text))
            {
                try
                {
                    transfer.SendString(txtEnvio.Text);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.ToString());
