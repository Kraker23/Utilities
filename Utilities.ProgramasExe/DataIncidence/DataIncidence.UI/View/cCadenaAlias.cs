...
{
    public partial class cCadenaAlias : UserControl
    {
        ...
        {
            CargarDatos(GetQuery(), "Cadena Alias");
        }
        ...
        private void Cargar(string Query)
        {
            ...
            {
                txtIdCadenaAlias.Text   = dt.Rows[0].ItemArray[0].ToString();
                txtTvCampana.Text       = dt.Rows[0].ItemArray[1].ToString();
                txtIdCadena.Text        = dt.Rows[0].ItemArray[2].ToString();
                txtCodCadenaAlias.Text = dt.Rows[0].ItemArray[3].ToString();
            }
            else
            {
                dtResult = dt;
                //EventoCargarDatos(dt, "CadenaAlias");
        ...
        private string GetQuery()
        {
            ...
            resultado = "  select idcadenaAlias,idtvcampana,idcadena,codcadenaalias from [dbo].[PLTVCadenaAlias] ";
            if (!string.IsNullOrEmpty(txtIdCadenaAlias.Text     ) ||
                !string.IsNullOrEmpty(txtTvCampana.Text      ) ||
                !string.IsNullOrEmpty(txtIdCadena.Text       ) ||
                !string.IsNullOrEmpty(txtCodCadenaAlias.Text) )
            {
                ...
                if (!string.IsNullOrEmpty(txtIdCadenaAlias.Text))
                {
                    param = param.añadirString("idcadenaAlias ='" + txtIdCadenaAlias.Text + "'", ';');
                }
                if (!string.IsNullOrEmpty(txtTvCampana.Text))
                {
                    param = param.añadirString("idtvcampana ='" + txtTvCampana.Text + "'", ';');
                }
